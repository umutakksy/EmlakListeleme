...
public class EmlakUI implements Initializable {
    ...
    private void aramaYap(KeyEvent event) {
        String aramaMetni = txtAra.getText().toLowerCase().trim();
        ...
        ObservableList<Emlak> filtreliListe = FXCollections.observableArrayList();
        
        for (Emlak emlak : emlakManager.getEmlakListesi()) {
            boolean eslesmeVar = 
                (emlak.getBaslik() != null && emlak.getBaslik().toLowerCase().contains(aramaMetni)) ||
                (emlak.getTip() != null && emlak.getTip().toLowerCase().contains(aramaMetni)) ||
                (emlak.getDurum() != null && emlak.getDurum().toLowerCase().contains(aramaMetni)) ||
                (emlak.getSehir() != null && emlak.getSehir().toLowerCase().contains(aramaMetni)) ||
                (emlak.getAdres() != null && emlak.getAdres().toLowerCase().contains(aramaMetni)) ||
                String.valueOf(emlak.getFiyat()).contains(aramaMetni) ||
                String.valueOf(emlak.getMetrekare()).contains(aramaMetni) ||
                (emlak.getOdaSayisi() != null && emlak.getOdaSayisi().toLowerCase().contains(aramaMetni));
                
            if (eslesmeVar) {
                filtreliListe.add(emlak);
            }
        }
        
        tabloGoruntuleme.setItems(filtreliListe);
    }
